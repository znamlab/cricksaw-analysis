...
if ADD_CELLS:
    ...
    cell_colors = {layer: color for layer, color in zip(layers, colors / 255)}
...
for layer in layers:
    ...
    if ADD_CELLS:
        cells_in_layer[layer] = cells_scaled[np.isin(cell_atlas_id, layer_index), :]
    ...
    data_dorsal_by_layer[layer] = reg_data_view
    bg_dorsal_by_layer[layer] = bg_data_view
    # new top and iterate
...
if NAPARI:
    for layer in layers[::-1]:
        viewer.add_image(
            data=bg_dorsal_by_layer[layer],
            name="Background layer %s" % layer,
            colormap="green",
        ...
        viewer.add_image(
            data=data_dorsal_by_layer[layer],
            name="Signal layer %s" % layer,
            colormap="gray",
        ...
        viewer.add_labels(
            data=atlas_dorsal_by_layer[layer],
            name="atlas %s" % layer,
            opacity=0.1,
            visible=True if layer == "1" else False,
        )
        if ADD_CELLS:
            flat_cell = np.array(cells_in_layer[layer])
            flat_cell[:, :] = flat_cell[:, [1, 0, 2]]
            flat_cell[:, 0] = 0
            viewer.add_points(
                data=flat_cell,
                name="Cells in %s" % layer,
                size=[1, 3, 3],
                symbol="disc",
                edge_width=0.1,
                face_color=cell_colors[layer],
            )
