...
def cell_in_volume(cell_coords, volume_mask, cell_scale_um, volume_scale_um):
    ...
    px_cells = np.array(
        np.round(cell_coords * cell_scale_um / volume_scale_um), dtype=int
    )
...
def find_cell_in_injection_site(
    project,
    mouse,
    channel,
    pixel_size_um,
    ...
    cell_xml = (
        processed
        / ("cellfinder_results_%03d" % pixel_size_um)
        / "points"
    ...
    cell_in_inj = cell_in_volume(
        cells[0], inj_site, cell_scale_um=cell_scale_um, volume_scale_um=pixel_size_um
    )
    cell_out_inj = cell_in_volume(
        cells[0], ~inj_site, cell_scale_um=cell_scale_um, volume_scale_um=pixel_size_um
    )
    ...
    if view_in_napari:
        df_kwargs = dict(
            ...
            symbol="disc",
            edge_width=0.5,
            face_color=np.array([190, 174, 212, 125.0]) / 255,
        )
