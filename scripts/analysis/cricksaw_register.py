...
elif machine.startswith("int") or machine.startswith("ca") or machine.startswith("vis"):
    # we are on camp
    atlas_directory = (
...
if do_trans_template:
    ...
    float_atlas, translator = atlas_utils.create_float_atlas(
        path2atlas, path2floatatlas
    )
...
for mouse_name in todo:
    if not os.path.isdir(os.path.join(raw_directory, mouse_name)):
        ...
        continue
    print("\n\nDoing %s\n" % mouse_name)
    raw_root_path = os.path.join(raw_directory, mouse_name, subfolder)
    processed_root_path = os.path.join(processed_directory, mouse_name)
    if not os.path.isdir(processed_root_path):
        os.mkdir(processed_root_path)
    roiPtsFiles = []
    cellPtsFiles = []
    ...
    if do_roi:
        if not os.path.isdir(masivPath):
            ...
            for fname in os.listdir(masivPath):
                ...
                if fname.endswith(".yml") and any(
                    [m in fname for m in ("injSite", "injsite", "track")]
                ):
    ...
    if do_roi:
        ...
        if roiPtsFiles:
            ...
            for roi_path in roiPtsFiles:
                ...
                target = os.path.join(
                    path2reg,
                    "%s_downsampled_%ium_roi%s" % (mouse_name, atlas_size, roi_name),
                )
                fulltarget = os.path.join(
                    path2reg,
                    "%s_fullres_roi_%s"
                    % (mouse_name, roi_name.replace(".xml", ".pts")),
                )
                roi_dict = pre_processing.downsample_brainsaw_roi(
                    roi_path,
                    path2recipe=recipeFile,
                    target=target,
                    full_res_target=fulltarget,
                    outsize=atlas_size,
                    force=True,
                )
                ...
                for ctype, roi_coords in roi_dict.items():
                    target = os.path.join(
                        path2reg,
                        "%s_%s_downsampled_%ium_step00_part00.pts"
        ...
        if cellPtsFiles:
            ...
            for cell_path in cellPtsFiles:
                ...
                target = os.path.join(
                    path2reg,
                    "%s_downsampled_%ium_cell_%s"
                    % (mouse_name, atlas_size, cell_name.replace(".xml", ".yml")),
                )
                fulltarget = os.path.join(
                    path2reg,
                    "%s_fullres_cell_%s"
                    % (mouse_name, cell_name.replace(".xml", ".pts")),
                )
                cell_dict = pre_processing.downsample_brainsaw_roi(
                    cell_path,
                    path2recipe=recipeFile,
                    target=target,
                    full_res_target=fulltarget,
                ...
                for ctype, cell_coords in cell_dict.items():
                    ...
                    rois_io.write_pts_file(
                        target,
                        np.asarray([c[0] for c in cell_coords]),
                        np.asarray([c[1] for c in cell_coords]),
                        np.asarray([c[2] for c in cell_coords]),
                        force=True,
                        index=False,
                    )
    ...
    if do_downsample:
        ...
        for cN in chanNames:
            target = os.path.join(
                path2reg,
                "%s_downsampled_%ium_chan%s.mhd" % (mouse_name, atlas_size, cN),
            )
            print("Downsampling channel %s..." % cN)
            pre_processing.downsample_brainsaw(
                path2tiffs=os.path.join(dataPath, cN),
                path2recipe=recipeFile,
                outsize=atlas_size,
                target=target,
                check_for_ram=ramcheck,
            )

    if do_register:
        ############# STEP 01 ##################
        movingImageName = path2template
        fixedImage = os.path.join(
            path2reg,
            "%s_downsampled_%ium_chan%s.mhd" % (mouse_name, atlas_size, chan2use),
        )

        transformedImage = register_functions.registration_single_step(
            fixedImage,
            movingImageName,
    ...
    if do_roi:
        ...
        registered_cells = register_functions.register_roi(
            path2reg, cell2register, mouse_name
        )

        if needs_LR_swap[mouse_name]:
            # I need to find the things registered in the atlas and swap left/right
            register_functions.swap_hem_atlas_roi(
                transform_folder=path2reg,
                roi_files=registered_rois,
        ...
        if do_roi_processing:
            ...
            for base_name, (raw_roi_file, reg_roi_file) in roi_to_process.items():
                # Find depth
                depth_dict = process_rab_roi.find_depth(
                    reg_roi_file, atlas_path=atlas_directory
                )
                for what, value in depth_dict.items():
                    np.save(os.path.join(path2reg, base_name + "_%s.npy" % what), value)

                # find cortical cells
                out = process_rab_roi.find_cortical_cells(
                    reg_roi_file, raw_roi_file, path2atlas, path2json
                )
    ...
    if do_trans_template:
        ...
        )  # was atlas_size / 25 when I register at 25 and transform at 10
        ...
        reg_float_atlas, path_to_reg_float = register_functions.apply_registration(
            path2reg,
            path2floatatlas,
