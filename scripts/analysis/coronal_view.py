...
for _, mouse in mice.iterrows():
    ...
    cellfinder_data = children[
        (children.dataset_type == "cellfinder") & (children.atlas == atlas_name)
    ]
    ...
    trans_atlas = tifffile.imread(
        cellfinder_data.path_full / "registration" / "registered_atlas.tiff"
    )
    trans_atlas = bg_space.map_stack_to(
        source_origin, target_origin, trans_atlas, copy=False
    )
    trans_hem = tifffile.imread(
        cellfinder_data.path_full / "registration" / "registered_hemispheres.tiff"
    )
    trans_hem = bg_space.map_stack_to(
        source_origin, target_origin, trans_hem, copy=False
    )
    ...
    for iax, parent_area in enumerate(areas):
        ...
        for channel in [2, 3]:
            ...
            for zslice in range(plane_per_section):
                tif = chan_dir / (
                    "section_%03d_%02d.tif" % (best_slice + 1, zslice + 1)
                )
        ...
        label_img = np.asarray(
            label_img.resize(rgb.shape[1::-1], resample=PIL.Image.NEAREST)
        )
        ...
        if parent_area in mprop:
            ...
            # crop img_data
            rgb = rgb[
                prop["ylim"][1] : prop["ylim"][0], prop["xlim"][0] : prop["xlim"][1], :
            ]
            label_img = label_img[
                prop["ylim"][1] : prop["ylim"][0], prop["xlim"][0] : prop["xlim"][1]
            ]
        ...
        o = atlas_utils.plot_borders_and_areas(
            ...
            areas_to_plot=[],
            color_kwargs=dict(),
            border_kwargs=dict(colors="white", lw=0.5),
